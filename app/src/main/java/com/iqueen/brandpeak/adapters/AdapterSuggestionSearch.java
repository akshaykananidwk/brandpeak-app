...
public class AdapterSuggestionSearch extends RecyclerView.Adapter<AdapterSuggestionSearch.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        holder.iv_clear.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                deleteSearchHistory(items.get(pos));
    ...
    public void deleteSearchHistory(String s) {
        List<String> searchHistory = getSearchHistory();
        if (searchHistory.contains(s)) {
            searchHistory.remove(s); // Remove the specified item
            SearchObject searchObject = new SearchObject(searchHistory); // Create an updated SearchObject
            String json = new Gson().toJson(searchObject, SearchObject.class);
            prefs.setString(SEARCH_HISTORY_KEY, json); // Save updated search history back to preferences
        }
    }
}